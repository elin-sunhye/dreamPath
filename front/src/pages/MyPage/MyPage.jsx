...
function MyPage(props) {
    ...
    const handelAuthPhoneOnClick = async () => {
        ...
        try {
            const response = await sendAuthPhoneMutation.mutateAsync(
    ...
    const handelConfirmButtonOnClick = async () => {
        await confirmPhoneNumberMutation
            .mutateAsync(phoneNumberInputValue)
            .then((response) => {
                ...
                <section css={s.profileSection}>
                    ...
                    <div css={s.profileContent}>
                        <label css={s.profileImage}>
                            {loginUser.isLoading || (
                                <img
                                    src={`https://dreampathpj.store/image/user/profile/${loginUser?.data?.data.profileImg}`}
                                    alt=""
                                />
                            )}
                            <input
                ...
                <section css={s.infoSection}>
                    ...
                    <div css={s.infoContent}>
                        ...
                        <div css={s.infoRow}>
                            <span>휴대폰 번호</span>
                            <input
                                onChange={handleAuthPhoneOnChange}
                                type="tel"
                            />
                            <button
                                onClick={handelAuthPhoneOnClick}
                                disabled={isSendMail}
                            >
                                인증하기
                            </button>
                            {authNumber !== '' ? (
                                <input
                                    onChange={handleAuthPhoneCheckOnChange}
                                    type="text"
                                />
                            ) : (
                                <></>
                            ...
                            <button
                                disabled={!isEqualCode}
                                onClick={handelConfirmButtonOnClick}
                            >
                                확인
                            </button>
                        </div>
